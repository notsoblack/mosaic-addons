...
const LoopsPanel: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (addonAPI?.agents?.list || addonAPI?.mcp?.listServers) {
      setApi(addonAPI);
    }
  }, []);
    ...
    <div className="h-full w-full flex flex-col bg-gray-950">
      ...
      {bridgeMissing && (
        <div className="px-3 py-2 bg-amber-900/30 border-b border-amber-700/50 text-[10px] text-amber-300 flex items-center gap-2">
          ...
          <span>No addon bridge detected — this addon is running outside Mosaic's addon runtime. Showing demo data.</span>
        </div>
      )}
      {/* ── Header ───────────────────────────────────────────────────────── */}
